...
class Word:
        ...
        def write(self):
            ...
            for i in range(len(self.word)):
                self.recipient.display.blit(FONT[self.word[i]], pygame.Rect(left + i * 3, self.place.top, FONT[self.word[i]].get_rect().width, FONT[self.word[i]].get_rect().height))
...
class Player:
    ...
    def save(self):
        ...
        for room in rooms.rooms:
            room.prepareToSave()
            self.prepareToSave()
            roomCoordsInfo[f'[{room.coordinate[0]}, {room.coordinate[1]}, {room.coordinate[2]}]'] = tuple(tuple(room.saveCode) + tuple(self.saveCode))
        try:
...
class Room:
    ...
    def prepareToSave(self):
        for foe in self.foes:
            foe.prepareToSave()
        self.saveCode = [foe.saveCode for foe in self.foes] + [resource.saveCode for resource in self.destructibleMaterialSources if resource.hp > 0] + [teleporter.saveCode for teleporter in self.teleporters]
...
class destructibleMaterialSource:
    def __init__(self, centerx, centery, type):
        ...
        self.place.centerx = centerx
        self.place.centery = centery
        self.saveStatTypes = [key for key in vars(self) if key != 'sprite' and key != 'place']
       ...
       abs(a.place.centery - b.place.centery) < (a.place.height + b.place.height) / 2:
            ...
            if type(a) == Bullet:
                if type(b) == Foe:
                    ...
                    for element in [element for element in a.elements]:
                        b.recentElementDamages.append(element)
                if a.delay < 1:
...
for i in range(4):
    if f'delete{i + 1}' == deleted:
        ...
        json.dump(['NanotechRevolver', 1, 1] + ["empty", 1, 1] * 99, inv)
        inv.close()
...
try:
    playerInfo = open(f'playerInfo{file}.pickle', 'x')
    playerInfo.close()
except FileExistsError:
    pass
def save():
    try:
        roomInfo = open(f'roomInfo{file}.pickle', 'x')
    except FileExistsError:
        roomInfo = open(f'roomInfo{file}.pickle', 'w')
    roomInfo.close()
    try:
        playerInfo = open(f'playerInfo{file}.pickle', 'x')
    except FileExistsError:
        playerInfo = open(f'playerInfo{file}.pickle', 'w')
    pickle.dump([p.inventory, p.hp, p.room, p.center], playerInfo)
    playerInfo.close()
def load():
    try:
        with open(f'playerInfo{file}.pickle', 'rb') as playerInfo:
            p2 = pickle.load(playerInfo)
            p.inventory = p2[0]
            p.hp = p2[1]
            p.room = p2[2]
            p.center = p2[3].copy()
        with open(f'roomInfo{file}.pickle', 'rb') as roomInfo:
            rooms2 = pickle.load(roomInfo)
            rooms.rooms = rooms2.rooms
            rooms.roomCoordinates = rooms2.roomCoordinates
    except EOFError:
        rooms.makeRooms(16, 'desert')
